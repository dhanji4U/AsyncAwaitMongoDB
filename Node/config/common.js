...
const setting = require("../modules/schema/setting_schema");
...
const common = {
    ...
    /*
    ** Function to get setting information
    */
    async settingDetails() {
        try {
            ...
            if (result.length > 0) {
                ...
                console.log("Setting Error: No settings found");
                return null;
            }
        } catch (error) {
    ...
    */
    ...
    /*
    ** Common function get where condition for filtering
    */
    async getFilterCondition(request, type) {
        ...
        if (request.search !== undefined && request.search !== null && request.search != '') {
            wherecondition.$or = [
                ...
                { address: { $regex: request.search, $options: 'i' } },
                { title: { $regex: request.search, $options: 'i' } },
                { url: { $regex: request.search, $options: 'i' } },
            ];
        }

        if (request.start_date != undefined && request.start_date != '' && request.start_date != null) {
            const formattedStartDate = moment(request.start_date).startOf('day').toDate();
            wherecondition.created_at = { $gte: formattedStartDate };
        }

        if (request.end_date != undefined && request.end_date != '' && request.end_date != null) {
            const formattedEndDate = moment(request.end_date).endOf('day').toDate();
            if (wherecondition.created_at) {
                wherecondition.created_at.$lte = formattedEndDate;
            } else {
                wherecondition.created_at = { $lte: formattedEndDate };
            }
        }
        ...
        if (request.is_active !== undefined && request.is_active !== null) {
            wherecondition.is_active = request.is_active;
        }

        if (request.login_status !== undefined && request.login_status !== null) {
            wherecondition.login_status = request.login_status;
        }
